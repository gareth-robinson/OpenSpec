...
Provide validation capabilities for verifying implementation correctness against main specs in `openspec/specs/`. The `/opsx:validate` command complements `/opsx:verify` by checking current implementation against the spec source of truth, rather than validating change artifacts.
